...
int main(int argc, const char **argv) {
  ...
  if (!face_cascade.load(face_cascade_name)) {
    printf("--(!)Error loading face cascade, please change face_cascade_name "
           "in source code.\n");
    return -1;
  };
  ...
  createCornerKernels();
  ellipse(skinCrCbHist, cv::Point(113, 155), cv::Size(23, 15), 43.0, 0.0, 360.0,
...
void findEyes(cv::Mat frame_gray, cv::Rect face) {
  ...
  cv::Rect leftEyeRegion(face.width * (kEyePercentSide / 100.0), eye_region_top,
...
cv::Mat findSkin(cv::Mat &frame) {
  ...
  for (int y = 0; y < input.rows; ++y) {
    ...
    for (int x = 0; x < input.cols; ++x) {
      cv::Vec3b ycrcb = Mr[x];
      //      Or[x] = (skinCrCbHist.at<uchar>(ycrcb[1], ycrcb[2]) > 0) ? 255 :
      //      0;
